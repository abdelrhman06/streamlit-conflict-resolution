...
if uploaded_file:
    ...
    # استخراج المعلومات من كود الجلسة
            ...
            if not student_info.empty:
                ...
                def find_alternative_group(day, time):
                    ...
                    for _, group in possible_groups.iterrows():
                        ...
                        session_time = group["Event Start Time"]
                        
                        if session_code == old_group:
                            continue  
                        
                        if physical_group_time is not None:
                            session_datetime = datetime.combine(datetime.today(), session_time)
                            physical_datetime = datetime.combine(datetime.today(), physical_group_time)
                            if abs((session_datetime - physical_datetime).total_seconds() / 3600) < 2.5:
                                continue  
                        ...
                        if 15 < group_counts[session_code] < 35:
                            ...
                            return session_code, session_time, group_counts[session_code]
                    return None, None, None
                ...
                session_requests.loc[session_requests["Username"] == username, [
                    "New Group", "New Group Time", "New Group Student Count", "Old Group", "Old Group Time", "Physical Group Time"
                ]] = [
                    new_group, new_group_time, new_group_count, old_group, old_group_time, physical_group_time
                ]
    ...
    st.download_button("📥 تنزيل التقرير", data=processed_data, file_name="session_requests_report.xlsx", mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
