...
# عنوان التطبيق
st.title("\U0001F4CA نظام حل تعارض الجلسات")
...
if uploaded_file:
    ...
    def find_alternative_group(day, time, level, language, grade, old_group):
        possible_groups = groups[(groups["Level"] == level) &
        ...
        for _, group in possible_groups.iterrows():
            ...
            if session_code not in group_counts:
                group_counts[session_code] = connect_sessions_l1[connect_sessions_l1["Session Code"] == session_code].shape[0]
    ...
    for session_requests, sheet_name, connect_sessions in zip(
        ...
        for _, row in session_requests.iterrows():
            ...
            if not student_info.empty:
                ...
                # الحصول على الجلسة الفعلية (Physical)
                ...
                physical_group_day = physical_info["Day"].values[0] if not physical_info.empty else None
                
                # البحث عن جروب بديل
                new_group, new_group_time, new_group_count = find_alternative_group(requested_day, requested_time, level, language, grade, old_group)
                
                # التحقق من التعارض مع الجلسة الفعلية (Physical)
                conflict = "No Conflict"
                if new_group and physical_group and physical_group_day == requested_day:
                    time_diff = abs((datetime.combine(datetime.today(), new_group_time) - physical_group_time).total_seconds()) / 3600
                    if time_diff < 2.5:
                        conflict = "Conflict Detected"
                
                # تحديث البيانات في الجدول
                session_requests.loc[session_requests["Username"] == username, [
                    "New Group", "New Group Time", "New Group Student Count", "Old Group", "Old Group Time", "Physical Group", "Physical Group Time", "Conflict"
                ]] = [
